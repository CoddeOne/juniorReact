...
class MiniCartCartImage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            active: 0,
    ...
    render() {
        return (
            <div className="miniCartAmountHolder" >
                <button className="miniAmountBtn miniAmountPlus " onClick={() => this.nextAmountof()}><BsPlusSquareFill size={25} /></button>
                <h3 className="miniNuvberOf">{this.state.amountOf}</h3>
                <button className="miniAmountBtn miniAmountMinus" onClick={() => this.prevAmoutOf()}><BsFileMinusFill size={27} /></button>
                ...
                {this.props.elem.gallery?.map((elem, i) => {
                    return (
                        <>
                            <img key={i}
                                src={elem}
                                className={i === this.state.active ? "curImg miniCartImages" : "miniCartImages"}
                                alt={"miniCartImage"} />
                        </>
                    )
                })}
                <button className="mimiCartBtn miniNext" onClick={() => this.nextImg()}><GrFormNext size={20} /></button>
                <button className="mimiCartBtn prev" onClick={() => this.prevImg()}><GrFormPrevious size={20} /></button>
...
export default connect(mapStateToProps)(MiniCartCartImage)